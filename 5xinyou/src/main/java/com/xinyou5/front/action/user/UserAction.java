package com.xinyou5.front.action.user;
...
public class UserAction extends BaseAction {
	...
	public String checkUserName() {
		...
		if (!StringUtils.isEmpty(remeberName)) {
			try {
				CookieUtils.addCookie(response, "yipinusername",encode.encode(URLEncoder.encode(username, "utf-8").getBytes()),10 * 24 * 60 * 60);
	...
	public String updateUserInfo() {
		try {
			...
			boolean flagIsNull = StringUtil.isEmpty(mobile)
					|| StringUtil.isEmpty(companyRegionId.toString())
					|| StringUtil.isEmpty(email);
			boolean flagReg = !CheckRegex.checkLength(contactName, 10)
					|| !CheckRegex.checkMobile(mobile)
					|| !CheckRegex.checkTel(tel)
					|| !CheckRegex.checkLength(companyAddress, 100)
				...
				if (flag) {
					if (!userService.repeatUsername(username)) {*/
						...
						if (User.STATUS_ABLE == user.getStatus()) {
							...
							sb.append(mailUrl)
									.append("/user/resetpassword.do?username=")
		...
		try {
			if (StringUtils.isNotEmpty(userinfo.getEmail())) {
				...
				if (mail.equals(user.getEmail())) {
					...
					// 验证其他邮箱
					boolean status;
					status = userService.repeatEmail(mail);
					if (status) {
						setMessage("status", "true");
						return JSONMSG;
					} else {
	...
	public void qqLogin() {
			...
			try {
				response.sendRedirect("http://openapi.qzone.qq.com/oauth/show?which=ConfirmPage&display=pc&response_type=code&client_id="+ appid +"&redirect_uri="+ redirectURI + "&scope=" + scope);
			} catch (IOException e) {
				e.printStackTrace();
			}
	}
	...
	public String verifyPhone() {
		...
		try {
			CustomUserDetails cuser = userService.getCurrentUser();
	...
	public String getVerifyCode() {
		...
		String smsCode = RandomNumUtil.getRandom(RandomNumUtil.NUM,
				SMS_CODE_LENGTH);
	...
	public String regSuccess() {
		try {
			...
			map.put("type", Point.POINTSOURCE_TEL);
	...
	public String saveSellerInForm() {
		try {
			userInfoService.saveSellerInForm(sellerInForm);
		} catch (YiPinStoreException e) {
			logger.error("提交商户入驻申请失败 :",
					e.getMessage());
		}
		return "sellerInSuccess";
	}
	...
	/**
	 * @return the sellerInForm
	 */
	public SellerInForm getSellerInForm() {
		return sellerInForm;
	}

	/**
	 * @param sellerInForm the sellerInForm to set
	 */
	public void setSellerInForm(SellerInForm sellerInForm) {
		this.sellerInForm = sellerInForm;
	}
