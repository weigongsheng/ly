 ...
 *
 * Examples at: http://waiting.iteye.com/blog/732186
 * @Author waiting  
 *
 ...
 * Modify by 朱一成
 * 增加 changeValuesByName 方法，参数及使用方法同 changeValues
 */
;
var LinkageSel = function(opts) {
	...
	this.outer = {
		 ...
		 * @param {bool}		是否触发用户自定义onchange回调函数, 默认TRUE
		 */
		changeValuesByName: function(parm, change) {
			//window.setTimeout(that._changeValues, 2000, parm, change, that);	// FIX 当表单中动态更新linkageSel并且 .dialog('open')时FF下页面闪动
			that._changeValuesByName(parm, change); // 2parm?
			return this;
		},
...
// 改变菜单选择项 trigger默认false
LinkageSel.prototype._changeValuesByName = function(parm, trigger, obj) {
	...
	if ( isNumber(parm) || typeof parm === 'string' ) {
		...
		if (parm.length >= 2) {
			var dataB = data[disA].cell;
			for (var b in dataB) {
				if (dataB[b].name == parm[1]) {
					disB = b;
				}
			}
			if (disB != '') {
			...
			if (parm.length >= 3) {
				var dataC = dataB[disB].cell;
				for (var c in dataC) {
					if (dataC[c].name == parm[2]) {
						disC = c;
					}
				}
				if (disC != '') {
	...
	that.resetTrigger(trigger, newParm);
		
	for (var i = 0; i < len; i++) {
		elm = bindEls[i]['obj'];
		if (elm.val() !== newParm[i]) {	// 如果数值与当前选项相同则不变更,直到第一个不相同的更改并退出循环
			elm && elm.find("option[value='" + newParm[i]  + "']").eq(0).prop('selected', true);		// for jQuery 1.6+
			break;
		}
	}
	elm.change();
