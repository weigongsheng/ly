...
//					var replaceQueueItem = confirm('文件命名 "' + file.name + '" 已经在队列.\n你想替换现有的项目在队列?');
//					if (!replaceQueueItem) {
			...
			if ($.inArray('onSelectError', settings.overrideEvents) < 0) {
				switch(errorCode) {
					...
					case SWFUpload.QUEUE_ERROR.FILE_EXCEEDS_SIZE_LIMIT:
						this.queueData.errorMsg += '\n所选文件 "' + file.name + '" 超过大小限制 (' + settings.fileSizeLimit + ').';
						break;
					case SWFUpload.QUEUE_ERROR.ZERO_BYTE_FILE:
						this.queueData.errorMsg += '\n所选文件 "' + file.name + '" 是空的.';
						break;
					case SWFUpload.QUEUE_ERROR.FILE_EXCEEDS_SIZE_LIMIT:
						this.queueData.errorMsg += '\n所选文件 "' + file.name + '" 不被接受的文件类型 (' + settings.fileTypeDesc + ').';
						break;
				}
			}
			...
			if (settings.checkExisting) {
				$.ajax({
					...
					success : function(data) {
						if (data == 1) {
							var overwrite = confirm('一个名字为 "' + file.name + '" 的文件已经存在服务器上.\n你想要取代现在的文件吗?');
							if (!overwrite) {
								this.cancelUpload(file.id);
								$('#' + file.id).remove();
